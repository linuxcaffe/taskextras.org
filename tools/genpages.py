...
def isvimwikibloghead(s):
    ...
    # 每行都只会是以下几种情况
    # Each line will be the following situations
    p=r'(^\*.+?\*$)|(^_.+?_$)|(^`.+?`$)'
    for line in s.splitlines():
        if not re.search(p,line.strip()):
            return False
    # 日期必须有,分类不能大于一个
    # Date must have, can not be greater than one classification
...
def genpages(gentpl,htmldict,attrs,indir,outdir):
    ...
    注意: 开头需满足一定格式
    Note: The beginning of the need to meet certain format
    '''
    ...
    for k,v in sorted(dcat.items(),reverse=True):
        ...
        for wiki in v:
            ...
            date = str.format('{0}In{1}Month{2}Day',a.time.tm_year,a.time.tm_mon,a.time.tm_mday)
